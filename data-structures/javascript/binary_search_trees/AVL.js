...
class AVL {
  ...
  checkBalance(currentNode) {
    if (currentNode.left === null && currentNode.right === null) {
      ...
      currentNode.height = Math.max(currentNode.left.height, currentNode.right.height);
    }
    console.log("GETTING HERE BEFORE")
    if ((currentNode.left.height - currentNode.right.height) > 1) {
      console.log("GETTING HERE")
      if ((currentNode.left.left.height - currentNode.left.right.height) > 0) {
        rightRotation(currentNode);
      } else {
        leftRotation(currentNode);
        rightRotation(currentnode);
      }
    } else if ((currentNode.left.height - currentNode.right.height) < -1) {
      console.log("GETTING HERE")
      if ((currentNode.right.left.height - currentNode.right.right.height) < 0) {
        leftRotation(currentNode);
      } else {
        rightRotation(currentNode);
        leftRotation(currentNode);
