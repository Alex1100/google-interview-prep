...
func TestGetPaymentMethodNewDebitCard(t *testing.T) {
  payment, err := GetPaymentMethod(NewDebitCard)
  if err != nil {
    t.Fatal("A payment method of type 'NewDebitCard' must exist")
  }
  msg := payment.Pay(22.30)
  if !strings.Contains(msg, "payed using debit card (new)") {
    t.Error("The new debit card payment method message wasn't correct")
  }
  t.Log("LOG: ", msg)
}
...
func TestGetPaymentMethodCreditCard(t *testing.T) {
  ...
  msg := payment.Pay(22.30)
  if !strings.Contains(msg, "payed using credit card implementation") {
